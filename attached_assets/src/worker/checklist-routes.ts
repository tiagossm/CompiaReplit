...
checklistRoutes.get("/checklist-templates", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (userProfile?.role === USER_ROLES.SYSTEM_ADMIN || userProfile?.role === 'admin') {
      ...
      console.log(`[TEMPLATES] [PROD] Admin bypass: role=${userProfile?.role}, email=${user.email}`);
    } else {
      // Para todos os outros usuários: templates públicos + próprios + da organização
      let conditions = ["ct.is_public = 1", "ct.created_by_user_id = ?"];
      params.push(user.id);
      
      if (userProfile?.organization_id) {
        conditions.push("ct.organization_id = ?");
        params.push(userProfile.organization_id);
        console.log(`[TEMPLATES] [PROD] User with org ${userProfile.organization_id} - incluindo templates públicos, próprios e da organização`);
      } else {
        console.log(`[TEMPLATES] [PROD] User sem organização - incluindo templates públicos e próprios`);
      }
  ...
  try {
    ...
    for (const field of fields) {
      if (field.field_name) {
        ...
        if (fieldsRequiringOptions.includes(field.field_type)) {
          ...
          if (field.options && field.options.trim() !== '') {
            try {
              if (typeof field.options === 'string' && field.options.startsWith('[')) {
                ...
                validOptions = field.options.filter((opt: any) => opt && opt.trim() !== '');
              }
            } catch (error) {
