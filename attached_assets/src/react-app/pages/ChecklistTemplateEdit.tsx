...
import ChecklistValidationHelper from '@/react-app/components/ChecklistValidationHelper';
import { 
  Save, 
  ArrowLeft, 
  Plus, 
...
export default function ChecklistTemplateEdit() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Update template
      const templateResponse = await fetch(`/api/checklist/checklist-templates/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(template)
      });

      if (!templateResponse.ok) throw new Error('Erro ao atualizar template');

      // Delete existing fields
      await fetch(`/api/checklist/checklist-templates/${id}/fields`, {
        method: 'DELETE'
      });
    ...
    if (updates.field_type && needsOptions(updates.field_type)) {
      ...
      try {
        if (currentField.options) {
          ...
          hasValidOptions = Array.isArray(parsed) && parsed.length > 0;
        }
      } catch {
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Validation Helper */}
          {fields.length > 0 && (
            <ChecklistValidationHelper fields={fields} />
          )}
                  ...
                  {needsOptions(field.field_type!) && (
                    <div className="mt-4">
                      ...
                      <textarea
                        ...
                        value={(() => {
                          try {
                            const currentValue = field.options ? JSON.parse(field.options).join('\n') : '';
                            // Se está vazio e é um campo que precisa de opções, usar valores padrão
                            if (!currentValue && needsOptions(field.field_type!)) {
                              const defaultOptions = field.field_type === 'multiselect' 
                                ? ['Adequado', 'Inadequado', 'Não Verificado', 'Não Aplicável']
                                : ['Conforme', 'Não Conforme', 'Não Aplicável'];
                              
                              // Atualizar o campo com valores padrão
                              setTimeout(() => {
                                updateField(index, { options: JSON.stringify(defaultOptions) });
                              }, 0);
                            ...
                            // Se está vazio, usar valores padrão
                            if (!currentValue && needsOptions(field.field_type!)) {
                              const defaultOptions = field.field_type === 'multiselect' 
                                ? ['Adequado', 'Inadequado', 'Não Verificado', 'Não Aplicável']
                                : ['Conforme', 'Não Conforme', 'Não Aplicável'];
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Campos do tipo "{fieldTypes.find(t => t.value === field.field_type)?.label}" requerem pelo menos uma opção
                      </p>
                    </div>
                  )}
                </div>
