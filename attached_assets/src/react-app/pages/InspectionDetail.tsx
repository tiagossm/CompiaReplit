          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <Link
                ...
                className="flex items-center px-3 sm:px-4 py-2 bg-purple-600 text-white text-sm rounded-lg hover:bg-purple-700 transition-colors"
              >
                <Target className="w-4 h-4 mr-1 sm:mr-2" />
                <span className="hidden sm:inline">Plano de</span> Ação
              </Link>
              <button
                onClick={generateAIAnalysis}
                disabled={aiAnalyzing || isSubmitting}
                className="flex items-center px-3 sm:px-4 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {aiAnalyzing ? (
                ...
                {aiAnalyzing ? 'Analisando...' : (
                  <>
                    <span className="hidden sm:inline">Análise IA (5W2H)</span>
                    <span className="sm:hidden">IA 5W2H</span>
                  </>
                )}
              </button>
              <button
                onClick={() => setShowAddItem(true)}
                className="flex items-center px-3 sm:px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors"
              >
                ...
                <ChecklistForm
                  fields={templateItems.map((item, index) => {
                    ...
                    return {
                      ...
                      field_name: fieldData.field_name || item.item_description,
                  ...
                  onAutoSave={async (formResponses, comments) => {
                    // Prevent multiple simultaneous auto-save calls
                    if (isSubmitting) return;
                    
                    try {
                      // Update local responses state immediately to prevent UI flicker
                      setResponses(prev => ({ ...prev, ...formResponses }));
                      
                      // Prepare field responses for API
                      const fieldResponses = Object.entries(formResponses).map(([fieldId, value]) => {
                        ...
                        try {
                          ...
                          return {
                            ...
                            field_name: fieldData.field_name || field.item_description,
                            field_type: fieldData.field_type,
                            value: value,
                            comment: comment
                          };
