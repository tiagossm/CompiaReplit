...
export default function NewInspection() {
  ...
  const fetchTemplates = async () => {
    try {
      console.log('[FRONTEND] Iniciando fetch de templates...');
      ...
      if (response.ok) {
        ...
        console.log('[FRONTEND] Templates brutos:', data.templates?.length || 0, data.templates);
        ...
        console.log('[FRONTEND] Templates válidos após filtro:', validTemplates.length, validTemplates);
        setTemplates(validTemplates);
      } else {
        const errorData = await response.json().catch(() => ({}));
        console.error('[FRONTEND] Erro na resposta da API:', response.status, response.statusText, errorData);
        setTemplates([]);
      }
    } catch (error) {
      console.error('[FRONTEND] Erro ao carregar templates:', error);
      setTemplates([]);
    }
  };
