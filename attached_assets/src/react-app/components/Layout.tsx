...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      {/* Mobile overlay - FIXED FOR PRODUCTION */}
      {sidebarOpen && (
        <div 
      ...
      {/* Sidebar - FIXED MOBILE OVERLAY ISSUE */}
      <div className={`
        fixed inset-y-0 left-0 z-50 w-72 bg-white shadow-xl border-r border-slate-100 transform transition-transform duration-300 ease-in-out flex flex-col
        ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
      `}>
        {/* Sidebar Header - Logo otimizada PRODUCTION */}
        <div className="flex items-center justify-center px-4 py-5 border-b border-slate-100 bg-white relative">
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="lg:hidden absolute top-3 right-3 p-1.5 text-slate-500 hover:text-slate-700 transition-colors bg-slate-100 rounded-md"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
                  ...
                  <>
                    ...
                    <li>
                      <Link
                        ...
                        className={`
                          flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200
                          ${location.pathname === '/admin/data-sync'
                            ? 'bg-gradient-to-r from-purple-50 to-purple-100 text-purple-700 shadow-sm border border-purple-200' 
                            : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'
                          }
                        `}
                      >
                        <Database className={`w-5 h-5 mr-3 ${location.pathname === '/admin/data-sync' ? 'text-purple-600' : 'text-slate-400'}`} />
                        Sinc. Dados
                      </Link>
                    </li>
                  </>
